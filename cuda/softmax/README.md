...
### 1.4 Online Softmax（单次遍历求 Max + Sum）
...
### 1.8 1 Block / Row + Block Reduce（Online Softmax 示例）

当 `col` 较大（例如几千到上万）时，可以让一个 block 负责一整行。  
每个 thread 扫描该行的一部分，先得到局部 `(m, d)`，再在 block 内做树形归约。
...
__global__ void online_softmax_block_kernel(const float* input, float* output, int row, int col) {
    int row_idx = blockIdx.x;  // 1 block -> 1 row
    if (row_idx >= row) return;

    const float* x = input + row_idx * col;
    float* y = output + row_idx * col;
    int tid = threadIdx.x;

    // 1) 每个线程扫描自己负责的列分片，得到局部 (m, d)
    MD local;
    local.m = -FLT_MAX;
    local.d = 0.f;
    for (int i = tid; i < col; i += THREADS) {
        MD elem{x[i], 1.f};
        local = md_combine(local, elem);
    }

    // 2) block 内树形归约 (m, d)
    __shared__ float smem_m[THREADS];
    __shared__ float smem_d[THREADS];
    smem_m[tid] = local.m;
    smem_d[tid] = local.d;
    __syncthreads();
    ...
    for (int stride = THREADS / 2; stride > 0; stride >>= 1) {
        if (tid < stride) {
            MD a{smem_m[tid], smem_d[tid]};
            MD b{smem_m[tid + stride], smem_d[tid + stride]};
            MD c = md_combine(a, b);
            smem_m[tid] = c.m;
            smem_d[tid] = c.d;
        }
    ...
    float row_m = smem_m[0];
    float inv_d = 1.f / smem_d[0];
...
- `THREADS` 取 `128/256/512` 做 benchmark 选最优。
- 若 `col` 特别大，可用 two-stage reduce（先 block 内，再跨 block）扩展。
- 可在 block 内先 warp reduce，再由 warp0 reduce，进一步减少同步开销。

### 1.9 Online Softmax + Top-K（Fused）

目标：在一次 kernel 中同时完成

1. online softmax 的 `(m, d)` 统计  
2. top-k 索引和值筛选  
3. 输出 top-k 的 softmax 概率

这样可以避免先写整行 softmax 再单独做 top-k 的额外读写。

核心流程（每行）：

- 每个线程扫描分片，维护：
  - `MD{m, d}`：online 归一化状态
  - `TopKPair[K]`：当前线程局部 top-k（按 logit 排序）
- block reduce：
  - `(m, d)` 用 `md_combine` 归约
  - `TopKPair[K]` 用 merge/insert 归约
- 线程 0 输出：
  - `idx_k = topk[k].idx`
  - `prob_k = exp(topk[k].logit - m) / d`

> 注意：top-k 选择应基于原始 logit 排序，而不是 softmax 后概率。因为 softmax 对同一行是单调变换，排序等价。
...
struct TopKPair {
    ...
    __device__ __forceinline__ void insert(float v, int id) {
        if (v <= val[K - 1]) return;
        val[K - 1] = v;
        idx[K - 1] = id;
        #pragma unroll
        for (int i = K - 2; i >= 0; --i) {
            bool better = (val[i + 1] > val[i]) || ((val[i + 1] == val[i]) && (idx[i + 1] < idx[i]));
            if (!better) break;
...
template <int K>
__device__ __forceinline__ TopKPair<K> merge_topk(const TopKPair<K>& a, const TopKPair<K>& b) {
    TopKPair<K> out = a;
    #pragma unroll
    for (int i = 0; i < K; ++i) out.insert(b.val[i], b.idx[i]);
    return out;
}
...
template <int K, int THREADS = 256>
__global__ void online_softmax_topk_fused_kernel(const float* input,
                                                 int* out_idx,
                                                 float* out_prob,
                                                 int row,
                                                 int col) {
    ...
    // 1) local scan: MD + local top-k
    MD local_md{-FLT_MAX, 0.f};
    TopKPair<K> local_topk;
    local_topk.init();

    for (int i = tid; i < col; i += THREADS) {
        float v = x[i];
        local_md = md_combine(local_md, MD{v, 1.f});
        local_topk.insert(v, i);
    }

    // 2) block reduce for MD and TopK
    __shared__ float smem_m[THREADS];
    __shared__ float smem_d[THREADS];
    __shared__ TopKPair<K> smem_topk[THREADS];
    smem_m[tid] = local_md.m;
    smem_d[tid] = local_md.d;
    smem_topk[tid] = local_topk;
    __syncthreads();
    ...
    for (int stride = THREADS / 2; stride > 0; stride >>= 1) {
        if (tid < stride) {
            ...
            smem_m[tid] = c.m;
            smem_d[tid] = c.d;
            smem_topk[tid] = merge_topk<K>(smem_topk[tid], smem_topk[tid + stride]);
        }
        __syncthreads();
    }

    // 3) thread0 writes final top-k index + probability
    if (tid == 0) {
        float m = smem_m[0];
        float inv_d = 1.f / smem_d[0];
        int* row_idx_out = out_idx + row_idx * K;
        float* row_prob_out = out_prob + row_idx * K;
        #pragma unroll
        for (int k = 0; k < K; ++k) {
            row_idx_out[k] = smem_topk[0].idx[k];
            row_prob_out[k] = expf(smem_topk[0].val[k] - m) * inv_d;
        }
    }
}
```
...
- 该版本适合 `K` 较小（如 `K<=8/16`）的场景，`K` 太大时寄存器和 shared memory 压力会上升。
- 若只需要 top-k 的 index，不需要概率，可跳过最后 `expf`，进一步省算力。
- tie-break（相同 logit）建议固定规则（上例用更小 index 优先），保证结果可复现。